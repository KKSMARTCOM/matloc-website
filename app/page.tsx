...
export default function Home() {
  const { t } = useI18n();
  const hero     = useCmsSettings("hero");
  const about    = useCmsSettings("about");
  const services = useCmsSettings("services");
  const partners = useCmsSettings("partners");
  ...
  return (
    <>
      ...
      <div className="relative w-full flex justify-center items-center h-150 md:h-200">
        <Image src={images.get("img_hero_bg", "/assets/images/jpg/Hero2.jpeg")} fill loading="eager"
          sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
          className="object-cover" alt="Hero" priority />
        ...
        <div className="p-6 space-y-6 absolute text-white text-center w-full md:w-[60%]">
          <h1 className="section-title">{hero.get("hero_title", t("home.heroTitle"))}</h1>
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-4">
            ...
            <Link href="/services" className="btn-outline-white block text-nowrap">
      ...
      <div className="container-site block md:flex items-center gap-12 py-20">
        <Reveal direction="right" className="relative w-full md:w-1/2 h-60 md:h-100 p-5">
          ...
          <div className="absolute inset-0 border-6 border-white bg-gray-300 w-full h-full shadow-xl rounded-md overflow-hidden">
            <Image src={images.get("img_about_home", "/assets/images/jpg/AboutHome.jpeg")} alt="About Image" fill
              sizes="(max-width: 768px) 100vw, 50vw"
              className="object-cover w-full h-full" />
          </div>
        </Reveal>
        <Reveal direction="left" distance={50} duration={1.3} delay={0.3}
          ...
          <div className="block sm:flex justify-between items-center">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="font-extrabold">{about.get("about_check1_title", t("home.maintenance"))}</h2>
                <p className="text-sm">{about.get("about_check1_sub", t("home.certified"))}</p>
              </div>
            </div>
            <div className="flex items-start gap-3 mt-6 sm:mt-0">
              <CheckCircleIcon size={20} className="text-primary" />
              <div>
                <h2 className="font-extrabold">{about.get("about_check2_title", t("home.flexibility"))}</h2>
                <p className="text-sm">{about.get("about_check2_sub", t("home.duration"))}</p>
              </div>
            </div>
          </div>
        </Reveal>
      ...
      <div className="py-20 bg-gray-50">
        ...
        <Image width={200} height={200} style={{ width: "200px", height: "200px" }}
            className="object-cover absolute -top-18 right-0 grayscale opacity-10"
            src="/assets/images/Icon.png" alt="Icone" />
          <SectionHeader
            title={services.get("services_title", t("home.services"))}
            subtitle={services.get("services_subtitle", t("home.servicesIntro"))}
          />
          <Reveal duration={1.5} delay={0.5} distance={80} className="flex flex-wrap justify-center gap-6">
            <ServicesSlider />
          </Reveal>
        </div>
      </div>
      ...
      <div className="py-20 bg-gray-50">
        <div className="container-site">
          <Reveal>
            <h1 className="section-title text-secondary mb-3">{t("home.quote")}</h1>
          </Reveal>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Reveal direction="right" duration={1.3} delay={0.3} distance={50} className="space-y-3">
