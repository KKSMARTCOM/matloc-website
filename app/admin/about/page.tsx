...
const EMPTY_MEMBER: Omit<DbMember, "id"> = {
  name: "",
  role: "",
  image_url: "",
  sort_order: 0,
};
const EMPTY_VALUE: Omit<DbValue, "id"> = {
  title: "",
  subtitle: "",
  sort_order: 0,
};
const MULTILINE_HOME = [
  "about_intro",
  "about_fleet",
  "about_check1_sub",
  "about_check2_sub",
];
const MULTILINE_PAGE = [
  "aboutpage_intro",
  "aboutpage_mission",
  "aboutpage_values_subtitle",
  "aboutpage_team_subtitle",
];
...
export default function AdminAboutPage() {
  ...
  const [memberModal, setMemberModal] = useState<{
    open: boolean;
    item: Partial<DbMember> & { id?: string };
  }>({ open: false, item: { ...EMPTY_MEMBER } });
  ...
  const [valueModal, setValueModal] = useState<{
    open: boolean;
    item: Partial<DbValue> & { id?: string };
  }>({ open: false, item: { ...EMPTY_VALUE } });
  ...
  return (
    <>
      ...
      {tab === "home" && (
        <FieldList
          fields={homeText.fields}
          loading={homeText.loading}
          onChange={homeText.handleChange}
          multilineKeys={MULTILINE_HOME}
          cols={2}
        />
      )}
      ...
      {tab === "page" && (
        <FieldList
          fields={pageText.fields}
          loading={pageText.loading}
          onChange={pageText.handleChange}
          multilineKeys={MULTILINE_PAGE}
          cols={2}
        />
      )}
      ...
      {tab === "values" && (
        <div>
          <div className="flex justify-end mb-4">
            <button
              onClick={() =>
                setValueModal({ open: true, item: { ...EMPTY_VALUE } })
              }
          ...
          {values.loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3">
              {values.items.map((v) => (
                <div
                  key={v.id}
                  className="bg-white rounded-xl border border-gray-200 p-4 hover:border-orange-200 transition-colors"
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button
                        onClick={() =>
                          setValueModal({ open: true, item: { ...v } })
                        }
                      ...
                      <button
                        onClick={() =>
                          setValueConfirm({ open: true, id: v.id })
                        }
      ...
      {tab === "members" && (
        <div>
          <div className="flex justify-end mb-4">
            <button
              onClick={() =>
                setMemberModal({ open: true, item: { ...EMPTY_MEMBER } })
              }
              className="flex items-center gap-1.5 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold rounded-lg transition-colors"
            >
          ...
          {members.loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {members.items.map((m) => (
                <div
                  key={m.id}
                  className="flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-3 hover:border-orange-200 transition-colors"
                >
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button
                      onClick={() =>
                        setMemberModal({ open: true, item: { ...m } })
                      }
                      className="p-1.5 rounded hover:bg-blue-50"
                    >
                      <Pencil size={15} className="text-blue-500" />
                    ...
                    <button
                      onClick={() => setMemberConfirm({ open: true, id: m.id })}
                      className="p-1.5 rounded hover:bg-red-50"
                    >
                      <Trash2 size={15} className="text-red-500" />
                    </button>
      ...
      <Modal
        ...
        onClose={() => setValueModal({ open: false, item: { ...EMPTY_VALUE } })}
        title={valueModal.item.id ? "Modifier" : "Nouvelle valeur"}
        size="sm"
      >
        <div className="space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            <button
              onClick={() =>
                setValueModal({ open: false, item: { ...EMPTY_VALUE } })
              }
              className="flex-1 px-4 py-3 text-[15px] font-semibold border border-gray-200 rounded-lg hover:bg-gray-50"
            >
              Annuler
            </button>
            <button
              onClick={async () => {
                const ok = await values.save(valueModal.item);
                if (ok)
                  setValueModal({ open: false, item: { ...EMPTY_VALUE } });
              }}
      ...
      <Modal
        ...
        onClose={() =>
          setMemberModal({ open: false, item: { ...EMPTY_MEMBER } })
        }
        title={memberModal.item.id ? "Modifier" : "Nouveau membre"}
        size="sm"
      >
        <div className="space-y-4">
          {[
            ...
            >
              ...
              <input
                type="text"
                value={(memberModal.item[k as keyof DbMember] as string) ?? ""}
                onChange={(e) =>
                  setMemberModal((m) => ({
                    ...m,
                    item: { ...m.item, [k]: e.target.value },
                  }))
                }
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              onClick={async () => {
                const ok = await members.save(memberModal.item);
                if (ok)
                  setMemberModal({ open: false, item: { ...EMPTY_MEMBER } });
              }}
