          ...
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-600 mt-0.5">
                Dernières modifications du contenu
              </p>
            </div>
            ...
            <ul className="divide-y divide-gray-50">
              {activity.map((a, i) => {
                ...
                return (
                  <li
                    key={i}
                    className="flex items-center gap-3.5 px-6 py-3.5 hover:bg-gray-50 transition-colors"
                  >
                    <div
            ...
            <>
              ...
              <div className="space-y-2.5">
                {STAT_CONFIG.slice(0, 5).map(({ label, key, color }) => {
                  const total = totals
                    ? STAT_CONFIG.slice(0, 5).reduce(
                        (s, c) => s + (totals[c.key as keyof Totals] as number),
                        0,
                      )
                    : 1;
                  const val = totals
                    ? (totals[key as keyof Totals] as number)
                    : 0;
