    ...
    <>
      ...
      {tab === "texts" && (
        <div className="space-y-6">
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Accueil — section &ldquo;Un parc matériel de pointe&rdquo;</p>
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Page /services — Titre hero</p>
        ...
        <div>
          ...
          {crud.loading ? <div className="flex justify-center py-12"><Loader2 size={20} className="animate-spin text-gray-400" /></div> : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {crud.items.map((s) => (
                <div key={s.id} className="bg-white rounded-xl border border-gray-200 p-3 hover:border-orange-200 transition-colors">
                  <div className="flex gap-3">
                    {s.image_url && <Image src={s.image_url} alt={s.title} width={56} height={42} className="w-14 h-11 object-cover rounded-lg shrink-0" unoptimized />}
                  ...
                  <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-gray-100">
                    <div className="flex gap-1 flex-wrap">
                      {s.points.slice(0, 2).map((p, i) => <span key={i} className="text-[10px] bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded-full">{p}</span>)}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => crud.save({ ...s, is_published: !s.is_published })}>
                        {s.is_published ? <ToggleRight size={18} className="text-green-500" /> : <ToggleLeft size={18} className="text-gray-400" />}
                      </button>
                      <button onClick={() => setModal({ open: true, item: { ...s } })} className="p-1 rounded hover:bg-blue-50"><Pencil size={13} className="text-blue-500" /></button>
                      <button onClick={() => setConfirm({ open: true, id: s.id })} className="p-1 rounded hover:bg-red-50"><Trash2 size={13} className="text-red-500" /></button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-1.5 mb-2">
              {(modal.item.points ?? []).map((p, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <button onClick={() => rmPoint(i)} className="p-1 text-red-400 hover:text-red-600"><Trash2 size={13} /></button>
                </div>
              ))}
            </div>
